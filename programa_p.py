        ...
        while True:
            ...
            if button == '-EditarTurma-':
                if values['-ListaDeTurmas-'] == 'Todas as turmas':
                    sg.popup_quick_message('Por favor selecione uma turma', font='123 25')
                    sleep(2)
                else:
                    self.window.Disable()
                    Editar_turma().star()
                    ...
                    while cancela == 0:
                        for c in ID_Turmas(definicoes[2]):
                            ...
                            if id != c[0]:
                                if values['-NDT-'] != c[1]:
                                    if values['-CLASS-']:
                                        criar_Turma(definicoes[2], 'Nenhum nome definido' if values['-NDT-'] == ''
                                                else values['-NDT-'], values['-TDT-'],
                                                    int(values['-Num-']), values['-DETALHE-'], ID=id)
                                        criar_aluno(definicoes[2], 'Nenhum nome definido' if values['-NDT-'] == ''
                                                    else values['-NDT-'], data, id)
                                        cancela = 1
                                    elif not values['-CLASS-']:
                                        criar_Turma(definicoes[2],
                                                    'Nenhum nome definido' if values['-NDT-'] == ''
                                                    else values['-NDT-'], values['-TDT-'],
                                                    self.maior + 1,
...
class Deficoes:
    def __init__(self):
        self.layout = [
            [sg.Text('Defina a lingua:'), sg.Combo(['Português', 'Inglês', 'Francês'],
                                                   size=(23, 1), default_value=definicoes[1])],
            [sg.Text('Defina um Tema:'), sg.Combo(['TanBlue', 'LightGreen3', 'LightBlue3',
                                                   'GreenTan', 'GreenMono', 'DarkTeal9', 'LightBrown11', 'LightGrey6',
                ...
                if button == 'Salvar':
                    ...
                    while True:
                        if value['-CorDeFundo-'] in lista_de_tema:
                            ...
                            sg.popup_quick_message('Por favor reinicie o programa', font='Calibri 20')
                            sleep(2)
                            break
                        elif value['-CorDeFundo-'] not in lista_de_tema:
                            sg.popup_quick_message('Por favor introduze um tema valido', font='Calibri 20')
                            sleep(2)
                            break
    ...
    def __init__(self):
        self.layout = [[sg.Text(f'Arquivo de dados actual: {definicoes[2]}')],
                       ...
                       [sg.InputText('', border_width=0, key='-Caminho-'),
                        sg.FilesBrowse('Procurar', file_types=(('Arquivos Cgest (.cgt)', '.cgt'),), key='-Procurar-')],
            ...
            if button in (None, 'Conluir', sg.WIN_CLOSED):
                if button == 'Conluir':
                    try:
                        ...
                        sg.popup_quick_message('O arquivo não existe.', font='123')
                        sleep(2)
                    self.window.close()
                break
            if value['-Procurar-']:
